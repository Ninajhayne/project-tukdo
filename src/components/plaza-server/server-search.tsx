...
export const ServerSearch = ({
    ...
    const onClick = ({ id, type }: {id: string, type: "channel" | "member"}) => {
        ...
        if(type === "member") {
            return router.push(`/plaza/servers/${params?.serverId}/conversations/${id}`);
        }

        if(type === "channel") {
            return router.push(`/plaza/servers/${params?.serverId}/channels/${id}`)
        }
    };
    ...
    return (
        <>
            <button
                className="group px-2 py-2 rounded-md flex items-center gap-x-2 w-full bg-[#00538a12] dark:hover:bg-[#00538a36]  hover:bg-[#00538a12] transition"
